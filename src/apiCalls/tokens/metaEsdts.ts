...
export const getMetaEsdtsCount =
  (props: ApiPropsType) =>
...
export const fetchMetaEsdts =
  ...
  async ({ address, page = 1, search, size }: FetchTokensArgumentsType) => {
    ...
    const getData = getMetaEsdts(props);
    const { data: newTokens } = await getData(params);
    return newTokens;
  };

export const fetchAllMetaEsdts =
  (props: ApiPropsType) => async (address: string) => {
    ...
    while (hasMoreTokens) {
      try {
        const getData = fetchMetaEsdts(props);
